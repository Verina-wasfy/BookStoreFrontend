...
import { AuthorsService } from 'src/app/_service/authors.service';
...
export class SaveBookComponent implements OnInit {
 ...
 bookForm = new FormGroup({
  ...
  private publishersService:PublishersService,private authorService:AuthorsService) {
 this.getAllLangs();
    this.getAllPubs();
    this.getAllAuths();

    this.book = data;
this.dateVal= new DatePipe('en').transform(this.book.publicationDate , 'MM/dd/yyyy')
    ...
    if (this.book != undefined) {
      ...
      this.bookForm.controls['publicationDate'].setValue(this.dateVal);
      this.bookForm.controls['publisherID'].setValue(this.book.publisherID);
      this.bookForm.controls['langID'].setValue(this.book.langID );
      this.bookForm.controls['authID'].setValue(this.book.bookAuth[0]?.authorID);
    }
   }
  ...
  getAllAuths() {
    this.authorService.getAllAuths().subscribe(result => {
      this.auths = result;
  ...
  onSubmit(form: any) {
   ...
   if(!this.bookForm.invalid){
     ...
     authEntity.authorID = form.controls['authID'].value;
